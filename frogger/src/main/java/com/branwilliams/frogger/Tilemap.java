...
public class Tilemap {
    ...
    public void forTilesInRange(float x, float y, float x1, float y1, TileConsumer consumer) {
        forTilesInRange(Mathf.floor(toTileX(x)), Mathf.floor(toTileY(y)), Mathf.ceil(toTileX(x1)),
                Mathf.ceil(toTileY(y1)), consumer);
    }
...
//        System.out.println(startX + ", " + startY + ", " + x1 + ", " + y1);
    ...
    public int toTileX(float x) {
        return (int) Math.max(x / (float) tileWidth, 0F);
    }

    public int toTileY(float y) {
        return (int) Math.max(y / (float) tileHeight, 0F);
    }

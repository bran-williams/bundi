...
public class VoxelScene extends AbstractScene implements Lockable {
    ...
    private boolean quitToMainMenu = false;

    public VoxelScene(VoxelRegistry voxelRegistry, VoxelTexturePack texturePack) {
        super("voxel_scene");
    ...
    public void init(Engine engine, Window window) throws Exception {
        ...
        voxelSoundManager = new VoxelSoundManager();
...
//        engine.setUpdateInterval(1 / 20D);
        ...
        es.addSystem(new PlayerWorldInteractSystem(this, this)); // This system implements its own lockable logic.
        ...
        if (voxelRegistry == null) {
            voxelRegistry = VoxelUtils.loadVoxelRegistry(jsonLoader, Paths.get("voxel_properties_hd.json"));
        }
        if (texturePack == null) {
    ...
    public void play(Engine engine) {
        ...
        Vector4f skyColor = new Vector4f(0F, 0F, 0.05F, 1.0F);

        Vector4f sunColor = new Vector4f(1.0F, 1F, 1F, 1.0F);
...
//        Vector4f sunColor = new Vector4f(1.0F, 0.9F, 0.7F, 1.0F);
    ...
    private void applySettings(Engine engine, Window window) {
        ...
        voxelSoundManager.getMusicSoundSource().setGain(gameSettings.getMusicVolume());
    }
...
//        VoxelChunkGenerator voxelChunkGenerator = new NoiseChunkGenerator(new PerlinNoise());
        VoxelChunkGenerator voxelChunkGenerator = new NoiseChunkGenerator(new LayeredNoise(new OpenSimplexNoise(),
                5), new OpenSimplexNoise());
    ...
    public void update(Engine engine, double updateInterval) {
        ...
        if (quitToMainMenu) {
            engine.setScene(new VoxelMainMenuScene());
        }
    }

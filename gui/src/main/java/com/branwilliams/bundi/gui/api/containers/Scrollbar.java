...
public class Scrollbar {
    ...
    private float maxScrollSpeed = 50F, minScrollSpeed = 1.693F; // 1.693 = 1 + log(2)
    ...
    public void wheel(int direction) {
        if (has()) {
            // velocity = -direction * (minSpeed + (log(1 + velocity / maxSpeed)))
            scrollVelocity += -direction * (minScrollSpeed + Mathf.log(2, 1 + (scrollVelocity / (maxScrollSpeed - minScrollSpeed))));
            constrict();
        }
    }
    ...
    /**
     * @return True if this scrollbar is moving at all.
     * */
    public boolean using() {
        return has() && (isScrolling() || scrollVelocity != 0F);
    }

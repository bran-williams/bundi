 ...
 */
public class AABB implements Shape2f {
    ...
    public boolean collides(Shape2f other) {
        if (other instanceof AABB) {
            ...
            return (getMaxX() > otherAABB.getMinX() && getMinX() < otherAABB.getMaxX()) &&
                    (getMaxY() > otherAABB.getMinY() && getMinY() < otherAABB.getMaxY());
    ...
    public Vector2f intersection(Shape2f other) {
        ...
        if (other instanceof AABB) {
            AABB otherAABB = (AABB) other;
            float dx = otherAABB.getCenter().x - this.getCenter().x;
            float px = (otherAABB.getHalfX() + this.getHalfX()) - Math.abs(dx);
            if (px <= 0) {
                return intersection;
            }

            float dy = otherAABB.getCenter().y - this.getCenter().y;
            float py = (otherAABB.getHalfY() + this.getHalfY()) - Math.abs(dy);
            if (py <= 0) {
                return intersection;
            }
    ...
    public boolean contains(Shape2f other) {
        if (other instanceof AABB) {
            AABB otherAABB = (AABB) other;
            return (getMinX() <= otherAABB.getMinX() && getMaxX() >= otherAABB.getMaxX()) &&
                    (getMinY() <= otherAABB.getMinY() && getMaxY() >= otherAABB.getMaxY());
        } else if (other instanceof Circle) {
            Circle otherCircle = (Circle) other;
    ...
    @Override
    public Shape2f copy() {
        return new AABB(this.min, this.max).center(this.center);
    }
